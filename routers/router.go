...
func InitRouter() *gin.Engine {
	...
	//下载图片
	project.StaticFS(setting.BedSetting.Link, http.Dir(setting.BedSetting.StorageLocation))

	//读取图片
	project.GET("/image/*mainUrl", api.GetImage)

	//检测并修复库中失效图片url
	project.GET("/autoCheckRepair", api.AutoCheckRepair)
	return r
}
